    ...
    def scan(s):
        for i in range(len(s) - len(pam) + 1):
            if s[i : i + len(pam)] == pam:
                start = i + len(pam)              # protospacer is adjacent to the PAM
                if start + L <= len(s):
...
def test_extraction_matches_slow_reference():
    ...
    expected = sorted(pack_guide(g, L) for g in _slow_extract(seq, L, pam))
    got = sorted(int(x) for x in extract_packed_guides(seq, L, pam))
    assert got == expected
    assert len(got) > 0  # the random seq should contain some TTT PAMs


def test_known_geometry():
    # PAM=TTT at index 0, spacer starts immediately after at index 3.
    seq = "TTT" + "ACGTAC"            # spacer = ACGTAC
    guides = extract_packed_guides(seq, 6, "TTT")
    decoded = {unpack_guide(int(x), 6) for x in guides}
    assert "ACGTAC" in decoded       # PAM-proximal first, forward strand


def _slow_skip(seq, L, pam):
...
def test_ambiguous_bases_are_skipped_not_folded():
    ...
    seq = "".join(seq)
    L, pam = 6, "TTT"
    want = sorted(pack_guide(g, L) for g in _slow_skip(seq, L, pam))
    got = sorted(int(x) for x in extract_packed_guides(seq, L, pam, "5prime"))
